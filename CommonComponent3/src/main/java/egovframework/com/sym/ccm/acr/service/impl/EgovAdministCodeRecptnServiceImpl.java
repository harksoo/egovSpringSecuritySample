			    ...
			    if(dataFile.exists()){
				    ...
				    while ( readData != null ) {
						...
						if (tokenLength >= 12) {
							...
							// 명령 변경구분코드로 변환
							strTmp = "INS".equals(tokenData[0]) ? "01"
								:  ( "UPD".equals(tokenData[0]) ? "02"
								:  ( "DEL".equals(tokenData[0]) ? "03"
								:    ""
								));
                            ...
                            // 상위기관코드 계산
                            String upperCode = administCodeRecptn.getSignguCode().equals("000") ? "" :
                            				 ( administCodeRecptn.getEmdCode()   .equals("000") ? administCodeRecptn.getCtprvnCode()+"000"+"000"+"00" :
                            				 ( administCodeRecptn.getLiCode()    .equals("00")  ? administCodeRecptn.getCtprvnCode()+administCodeRecptn.getSignguCode()+"000"+"00"
                            						                                            : administCodeRecptn.getCtprvnCode()+administCodeRecptn.getSignguCode()+administCodeRecptn.getEmdCode()+"00"
                            				 ));
